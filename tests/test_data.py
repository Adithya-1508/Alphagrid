...
def test_feature_store_resampling():
    ...
    weather_df = pd.DataFrame({"wind_speed_ms": [5.0]*len(weather_idx), "temperature_c": [12.0]*len(weather_idx)}, index=weather_idx)
    
    with patch("alphagrid.data.entsoe_client.get_wind_generation", return_value=wind_df), \
